...
const Login = () => {
  ...
  // state variable to store emailId and password
    ...
    try {
      if(state === 'Admin') {
        ...
        if(data.success){
          ...
          // store token 
          localStorage.setItem('aToken', data.token) // store token in local storage as when we refresh the page the token will be lost
    ...
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg'>
        ...
        <button className='bg-blue-500 text-white w-full py-2
         rounded-md text-base cursor-pointer '>Login</button>
